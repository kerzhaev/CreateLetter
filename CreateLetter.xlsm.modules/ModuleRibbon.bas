...
Public Sub RibbonSelectTemplateFolder(control As IRibbonControl)
    PromptAndSaveFolderSetting RibbonSettingTemplateFolder, _
                               t("ribbon.dialog.template_folder", "Выберите папку с шаблонами"), _
...
Public Sub RibbonSelectOutputFolder(control As IRibbonControl)
    PromptAndSaveFolderSetting RibbonSettingOutputFolder, _
                               t("ribbon.dialog.output_folder", "Выберите папку для сформированных писем"), _
                               t("ribbon.msg.output_folder_saved", "Папка для писем сохранена:")
End Sub
...
Private Function GetConfiguredFolderPath(settingKey As String) As String
    ...
    If Dir$(storedPath, vbDirectory) = "" Then
        GetConfiguredFolderPath = GetDefaultWorkbookFolderPath()
        Debug.Print t("ribbon.msg.folder_unavailable", "Путь недоступен, использован путь книги:") & " " & storedPath
        Exit Function
    End If
...
Private Function BuildAboutMessage() As String
    BuildAboutMessage = t("ribbon.about.name", "CreateLetter") & vbCrLf & vbCrLf & _
                        ...
                        t("ribbon.about.output_folder", "Папка выгрузки писем: ") & GetConfiguredOutputFolderPath() & vbCrLf & vbCrLf & _
                        t("ribbon.about.open_form_hint", "Используйте ленту Excel для открытия формы и настройки папок.")
End Function
