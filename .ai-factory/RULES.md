...
- Dispatch registry build scope must be current-registry scoped. `BuildDispatchRegistryFromDispatchItems` must not rebuild from every historical `tblDispatchItems` row; it should use `packed` packages as the target set, include `registered` packages only when they share the same registry number/date, and always exclude `registry_printed` plus blank legacy rows.
- Envelope layout preparation must be current-registry scoped. `PrepareEnvelopePrint` must prepare C4/C5/DL pages only for package `BatchId` values currently present in `tblDispatchRegistry`; it must not print every historical `tblDispatchItems` row.
- Legacy dispatch cleanup must be explicit and confirmed by the user. Rows with empty status plus empty registry number/date may be deleted only through the diagnostics cleanup path, and cleanup must clear related `tblLetters` dispatch tracking so those letters return to work.
- Safe dispatch-package rollback is allowed only before `registry_printed`. Rollback must clear the corresponding `tblLetters` tracking columns, delete the package rows from `tblDispatchItems`, and remove matching rows from `tblDispatchRegistry`.
...
- Do not mutate or rebind a UserForm `ListBox` inside its own `DblClick` event. Queue the action and run the same transfer code used by button clicks after the event returns; direct `RemoveItem`, `Clear`, or full rebind inside `DblClick` can disconnect MSForms objects and crash Excel.
- Do not use immediate `Application.OnTime Now` from a modal UserForm event. If a deferred UserForm action is unavoidable, schedule it at least one second later, store the scheduled timestamp, and cancel it in `UserForm_Terminate`.
...
- Excel print-layout sheets must be visually verified as PDF/PNG, not only smoke-tested. For envelope layouts, keep each package inside one printable page, compute `PrintArea` from the real used range instead of a sentinel column, and avoid placing required text in far-right columns that Excel may split into a second horizontal page.
- Envelope layout changes must keep a printerless verification path. The preview-grid mode should use the same current-registry data source as normal envelope preparation and add only visual control frames/labels, without changing dispatch lifecycle statuses.
