...
- Mail-dispatch lifecycle statuses are operational state, not display text: `packed` is set when a package is saved, `registered` when the internal registry is rebuilt, and `registry_printed` when the postal PDF is exported. Available-letter lists must exclude any letter that already has a package, registry number, registry date, or non-empty packed flag.
...
- Do not call `Clear`, `AddItem`, or full list-rebind logic on a UserForm ListBox from inside that same ListBox event. For double-click moves, use targeted `RemoveItem`/single-record updates or defer the rebind after the event returns to avoid MSForms automation disconnect errors.
