...
The `Addresses` worksheet now supports optional `AddressGroup` and `RPBS` fields. `AddressGroup` is a human search/explanation attribute for shared postal addresses, while `RPBS` is the stable servicing-financial-body key used by mail dispatch to allow different named recipients in one physical package.
...
│   ├── frmProgramSettings.frm           # Workbook-backed shared settings UI
